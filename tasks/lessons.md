...
- [2026-09-13] Neo 用固定音量阈值和转写事件自行静音，背景声触发后丢失播报内容，持续噪声还阻止恢复 → Live 1 负责插话、停说和回答时机；本地持续上传麦克风并播放远端音频，不设置自动静音、等待恢复或追加打断指令。回归覆盖持续及间歇噪声、转写事件乱序、双向音频与告别收尾；真人分别验证首次停声耗时和新回复内容，合成测试不能替代真实插话验收。
