async (page) => {
  ...
  const inject = () => page.addScriptTag({ url: page.url().replace(/\/tests\/.*$/, "/extension/content.js") + `?test=${Date.now()}` });
  ...
  await page.evaluate(() => {
    ...
    window.reply = (message, port) => port.receive({ ok: true, items: message.items.map(({id, text}) => ({id, text: `中文译文：${text}`})) });
    window.chrome = { runtime: { connect: () => {
      const port = {
        ...
        postMessage(message) { window.requests.push(message); queueMicrotask(() => window.reply(message, port)); },
      };
  ...
  check(result.count === 8 && result.batches.length === 2 && result.batches.every((n) => n === 4), "Expected 8 paragraphs in 2 batches");
  ...
  await page.evaluate(() => {
    window.requests = [];
    window.reply = (message, port) => { window.held = {message, port}; };
  });
  await inject();
  ...
  await page.evaluate(() => window.held.port.receive({ok:true, partial:true,
    text: '{"id":"p0","text":"第一段\\n路径 \\\\ 与引号 \\"测试\\"，Unicode \\u4f'}));
  check(await page.evaluate(() => document.querySelector("notype-translation")?.shadowRoot.querySelector("span").textContent === '第一段\n路径 \\ 与引号 "测试"，Unicode '), "Partial JSON escapes were decoded incorrectly");
  ...
  await page.evaluate(() => {
    ...
    window.held.port.receive({ok:true,partial:true,text:'{"id":"p0","text":"暂时译文'});
    heading.textContent = original;
    window.held.port.receive({ok:true,partial:true,text:'{"id":"p0","text":"恢复后的译文'});
  });
  check(await page.evaluate(() => document.querySelector("h1").nextElementSibling.shadowRoot?.textContent.includes("恢复后的译文")), "A restored source lost its translation view");
  await inject();
  await page.evaluate(() => window.held.port.receive({ok:true, items:window.held.message.items.map(({id})=>({id,text:"迟到译文"}))}));
  ...
  await page.evaluate(() => {
    ...
    window.held.port.receive({ok:true, items:window.held.message.items.map(({id})=>({id,text:"译文"})).reverse()});
  });
  ...
  await page.evaluate(() => {
    window.held.port.receive({ok:true,partial:true,text:'{"id":"p0","text":"暂时译文'});
    window.held.port.receive({ok:true,items:[{id:"wrong-id",text:"错误译文"}]});
  });
  ...
  // Off-screen paragraphs are deferred until scrolling near them; HTML-like replies stay text.
  await page.evaluate(() => {
    const paragraph = document.createElement("p");
    paragraph.id = "far-away";
    paragraph.style.marginTop = "4000px";
    paragraph.textContent = "This distant paragraph should wait until scrolling.";
    document.querySelector("main").append(paragraph);
    window.requests = [];
    window.reply = (message, port) => port.receive({ok:true,items:message.items.map(({id})=>({id,text:'<img src=x onerror="window.injected=true">'}))});
  });
  await inject();
  await page.waitForFunction(() => document.querySelector("notype-status")?.shadowRoot.textContent.includes("已翻译 8 / 9"));
  check(await page.evaluate(() => !window.requests.flatMap((r)=>r.items).some((item)=>item.text.includes("distant"))), "Off-screen text was sent early");
  check(await page.evaluate(() => !window.ports.at(-1).closed), "Incomplete page lost its reusable connection");
